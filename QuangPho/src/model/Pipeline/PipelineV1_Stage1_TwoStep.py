...
class TwoStepWheatNet(nn.Module):
    ...
    def __init__(self, dropout_rate=0.4):
        ...
        print("  2. MS Encoder (9→128 channels: 5 MS + 4 Indices, TRAINABLE)...")
        print("     Indices: NDVI, GNDVI, NDRE, MCARI")
        self.ms_encoder = nn.Sequential(
            nn.Conv2d(9, 32, 3, padding=1),  # ⭐ Changed from 5 to 9!
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2),
...
class Stage1Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, rgb_paths, ms_paths, hs_paths, labels, augment=False, use_handcrafted=True):
        ...
        # ⭐ Priority 4: Class-specific RGB augmentation
        if augment:
            ...
            self.rgb_transform_healthy = A.Compose([
                ...
                A.Rotate(limit=25, p=0.6),  # ⭐ Increased rotation
                A.RandomBrightnessContrast(brightness_limit=0.25, contrast_limit=0.2, p=0.5),  # ⭐ Stronger
                A.HueSaturationValue(hue_shift_limit=15, sat_shift_limit=20, val_shift_limit=15, p=0.4),  # ⭐ NEW
                A.GaussNoise(p=0.3),  # ⭐ NEW (removed var_limit)
    ...
    def __getitem__(self, idx):
        ...
        # ⭐ NEW: Compute spectral indices from MS
        ms_indices = self.compute_spectral_indices(ms)  # (H, W, 4)
        
        # ⭐ NEW: Concatenate MS + Indices: (H, W, 5) + (H, W, 4) = (H, W, 9)
        ms_with_indices = np.concatenate([ms, ms_indices], axis=-1)
        
        label = self.labels[idx]
        
        # ⭐ Apply class-specific RGB transform
        ...
        # ⭐ UPDATED: Transform MS+Indices together
    ...
    def __getitem__(self, idx):
        ...
        ms_transformed = self.get_ms_transform()(image=ms_with_indices)['image']  # ← 9 channels!
        hs_transformed = self.get_hs_transform()(image=hs)['image']
        ...
        if self.augment:
            ...
            if torch.rand(1) < 0.2:
                drop_idx = torch.randint(0, 9, (1,))  # ⭐ 5 → 9 channels!
...
if __name__ == "__main__":
    ...
    print("  ⭐ Spectral: MS (5) + Indices (4) = 9 channels → 128 features")
    print("    └─ Indices: NDVI, GNDVI, NDRE, MCARI")
    print("  - HS: 125 channels → 128 features")
    print("  - RGB weight: 65% (Spectral: 35%)")
    print("  - Loss: CrossEntropyLoss (class_weight=[1.0, 2.5])")
    print("  ⭐ Mixup: Enabled (alpha=0.4, 50% probability)")
    print("  - Oversample: HEALTHY 2x (balance 40→80, equal to Diseased)")
    print("  ⭐ Priority 4: Class-specific augmentation")
    print("    └─ Healthy: AGGRESSIVE (Rotate±25°, HSV, Noise)")
    print("    └─ Diseased: STANDARD (preserve disease features)")
    print("="*70 + "\n")
    ...
    for label in train_labels:
        if label == 0:  # Healthy (minority: 40 samples)
            sample_weights.append(2.0)  # Oversample 2x (40→80) to balance with Diseased
    ...
    print(f"  ✅ Healthy samples will be seen ~2x more often (balance 40→80)")
    print(f"  ✅ Expected Healthy samples per epoch: {train_labels.count(0) * 2}")
    ...
    print("  - Expected: Diseased Recall 65%→75%, Diseased→Healthy FN 35%→22-28%\n")
